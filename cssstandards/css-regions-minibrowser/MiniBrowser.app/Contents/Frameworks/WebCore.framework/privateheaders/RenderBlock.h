...
public:
...
#if ENABLE(REGIONS)
    int logicalRightOffsetForLine(int position, bool firstLine) const { return logicalRightOffsetForLine(position, logicalRightOffsetForRegion(position), firstLine); }
#else
    ...
    int availableLogicalWidthAt(int logicalOffset) const;
...
#if ENABLE(REGIONS)
    virtual bool hasRegions() const;
#else
    bool hasRegions() const { return false; }
#endif
...
#if ENABLE(REGIONS)
	int logicalRightOffsetForRegion(int position) const { return style()->isHorizontalWritingMode() ? borderLeft() + paddingLeft() + availableLogicalWidthAt(position) : borderTop() + paddingTop() + availableLogicalWidthAt(position); }
#endif
...
protected:
    ...
    void layoutPositionedObject(RenderBox* r, bool relayoutChildren);
...
private:
...
#if ENABLE(REGIONS)
    unsigned regionAtLogicalHeight(int height) const;
    unsigned regionAtLogicalWidth(int height) const;
    unsigned regionAtPosition(int position, int* regionLogicalHeight = 0) const;
    bool computeLineExclusionSegments(SegmentController* controller, int position, bool firstLine, bool& hasLineSegments) const;
#endif
    ...
    bool findNextLineBreak(InlineBidiResolver&, bool firstLine, bool& isLineEmpty, LineBreakIteratorInfo&, bool& previousLineBrokeCleanly,
                                     EClear*, FloatingObject* lastFloatFromPreviousLine, Vector<RenderBox*>& positionedObjects, Vector<InlineSegment>& segmentBreaks);
    RootInlineBox* constructLine(bool firstSegment, bool hasMultipleSegments, unsigned runCount, BidiRun* firstRun, BidiRun* lastRun, bool firstLine, bool lastLine, RenderObject* endObject);
    InlineFlowBox* createLineBoxes(RenderObject*, bool firstRun, bool firstSegment, bool hasMultipleSegments, bool firstLine);

    void computeInlineDirectionPositionsForLine(RootInlineBox*, InlineFlowBox*, float logicalLeft, float availableLogicalWidth, bool lastSegment, bool firstLine, BidiRun* firstRun, BidiRun* trailingSpaceRun, bool reachedEnd, GlyphOverflowAndFallbackFontsMap&, VerticalPositionCache&);
    void computeBlockDirectionPositionsForLine(RootInlineBox*, GlyphOverflowAndFallbackFontsMap&, VerticalPositionCache&);
