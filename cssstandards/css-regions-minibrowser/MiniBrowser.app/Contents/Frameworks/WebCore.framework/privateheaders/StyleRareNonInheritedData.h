...
public:
    ...
    int m_regionIndex;
    // Store the wrap shape mode.
    // Values: none | around | left | right | maximum | top-bottom | content
    // Initial value: none
    // Applies: block element
    // Inherited: none
    WrapShapeMode m_wrapShapeMode;
    
    // Store the wrap shape order
    int m_wrapShapeOrder;
    
    // The wrap shape
    RefPtr<PolygonShape> m_wrapShape;
    bool m_renderWrapShape;
    
    // Wrap shape margin and padding
    Length m_wrapShapeMargin;
    Length m_wrapShapePadding;
